...
static pcl::PointCloud<pcl::PointXYZ> map_cloud, add;
...
{
  LOG(INFO)<<"start to load map...1";
  if (map_loaded == 1) return;
  LOG(INFO)<<"start to load map...2";
  if (points_map_num != input->width)
  {
    ...
    pcl::fromROSMsg(*input, map_cloud);
    ...
    if (_use_local_transform == true)
    {
      ...
      pcl_ros::transformPointCloud(map_cloud, map_cloud, local_transform.inverse());
    }
    ...
    for(int i = 0; i < 20; i++){
      LOG(INFO)<<std::fixed<<std::setprecision(6)<<"evaleate:"<<map_cloud.points[i].x<<", "<<map_cloud.points[i].y;
    }
    pcl::PointCloud<pcl::PointXYZ>::Ptr map_ptr(new pcl::PointCloud<pcl::PointXYZ>(map_cloud));
    LOG(INFO)<<"set map to align_pointmatcher...";
    if(!align_pointmatcher_ptr->SetMap<pcl::PointXYZ>(map_ptr)){
    // if(!align_pointmatcher_ptr->setmap("")){
      LOG(FATAL)<<"failed to load localization map!";
    }
    LOG(INFO)<<"map load success.";
    map_loaded = 1;
  }
}
...
//INIT_init_pos_set = 1; callback of topic:"initialpose"
...
pcl::PointCloud<pcl::PointXYZ>::Ptr FilterCloudFrame(pcl::PointCloud<pcl::PointXYZ>::Ptr &input, 
    ...
    //voxel filter
    pcl::PointCloud<pcl::PointXYZ>::Ptr voxel_filtered_pts(new pcl::PointCloud<pcl::PointXYZ>);
    ...
    transform.rotate (Eigen::AngleAxisd (90*M_PI/180, Eigen::Vector3d::UnitZ()));
...
static void points_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  ...
  if (map_loaded == 1 && init_pos_set == 1)
  {
    ...
    common::Timer icp_time;
    LOG(INFO)<<std::fixed<<std::setprecision(6)
               <<"init guess:"<<init_guess(0,3)<<", "<<init_guess(1,3)<<", score:"<<fitness_score;
    if(!align_pointmatcher_ptr->AlignWithMap<pcl::PointXYZ>(filtered_scan_ptr, init_guess, fitness_score)){
      LOG(WARNING)<<"fa";
    }
    t = init_guess;
    LOG(INFO)<<std::fixed<<std::setprecision(6)
               <<"align result:"<<t(0,3)<<", "<<t(1,3)<<", score:"
               <<fitness_score<<", elapsed time:"<<icp_time.end()<<"[ms]";
  
    // if (_method_type == MethodType::PCL_GENERIC)
    // {
    //   align_start = std::chrono::system_clock::now();
    //   // init_guess = Eigen::Matrix4d::Identity(); //test  score is too large
    ...
    localizer_pose_pub.publish(localizer_pose_msg);//pub-topic: "/localizer_pose"  trans
    ...
    {
      br.sendTransform(tf::StampedTransform(transform, current_scan_time, "/map", "/base_link"));//ndt_pose result    map/base_link
    }
...
{
  ros::init(argc, argv, "icp_matching_neolix");
  ...
  //   std::string directory_name = "/tmp/Autoware/log/icp_matching_neolix";
  ...
  std::string _icp_config;
  private_nh.getParam("icp_config", _icp_config);
