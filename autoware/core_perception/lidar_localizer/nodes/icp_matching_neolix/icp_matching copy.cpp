...
static AlignPointMatcher::Ptr align_pointmatcher_ptr;
...
//INIT_init_pos_set = 1;  callback of topic:"config/ndt"
...
static void map_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  ...
  if (points_map_num != input->width)
  {
    ...
    for(int i = 0; i < 20; i++){
      LOG(INFO)<<std::fixed<<std::setprecision(6)<<"evaleate:"<<map.points[i].x<<", "<<map.points[i].y;
    }
...
static void points_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  ...
  if (map_loaded == 1 && init_pos_set == 1)
  {
    ...
    else if (_method_type == MethodType::ICP){
      if(!align_pointmatcher_ptr->AlignWithMap<pcl::PointXYZ>(filtered_scan_ptr, init_matrix,fitness_score)){
...
int main(int argc, char** argv)
{
  ...
  //init 
  align_pointmatcher_ptr.reset(_icp_config);
