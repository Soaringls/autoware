...
void CeresOptimizer::InsertOdom(const POSEPtr odom){
    if(!odom_init_flag) {
        ...
        odom_init_flag = true;
        ROS_INFO_STREAM("init W_GPS_TO_BODY's pose:"<<std::fixed<<std::setprecision(6)<<W_GPS_TO_BODY(0,3)<<","<<W_GPS_TO_BODY(1,3)<<","<<W_GPS_TO_BODY(2,3));
        return;
    }
    odom_mutex_.lock();
    ...
    // ROS_INFO_STREAM("opti W_GPS_TO_BODY's pose:"<<std::fixed<<std::setprecision(6)<<W_GPS_TO_BODY(0,3)<<","<<W_GPS_TO_BODY(1,3)<<","<<W_GPS_TO_BODY(2,3));
    ...
    // ROS_INFO_STREAM("opti tf           's pose:"<<std::fixed<<std::setprecision(6)<<odom->pos[0]<<","<<odom->pos[1]<<","<<odom->pos[2]);
    // ROS_INFO_STREAM("opti result_pose  's pose:"<<std::fixed<<std::setprecision(6)<<result_pose->pos[0]<<","<<result_pose->pos[1]<<","<<result_pose->pos[2]);
    W_GPS_TO_BODY.block<3,3>(0,0) = (result_pose->q).toRotationMatrix();
    W_GPS_TO_BODY.block<3,1>(0,3) = result_pose->pos;
    // ROS_INFO_STREAM("++++++++++optimized_data_:"<<result_pose->pos.x()<<", "<<result_pose->pos.y()<<", "<<result_pose->pos.z());
...
bool CeresOptimizer::GetOptimizedResult(POSEPtr& result){
    if(last_optimized_pose_ == nullptr) return false;
    result = last_optimized_pose_;
    return true;
}
...
void CeresOptimizer::Optimize(){
    ...
    while(1){
        if(flag_anchor_){
            ...
            ROS_INFO_STREAM("global optimization .....");
            ...
            ROS_INFO_STREAM("CeresOptimizer::Optimize() data's length:"<<length);
            ...
            for(int i = 0; i < length - 1; ++i){
                if(i + 1 < length) {
                    // ROS_INFO_STREAM("Create odom's cost func:["<<std::fixed<<std::setprecision(6)<<i<<":"<<odom_data_[i]->pos.x()<<", "
                    //                 <<odom_data_[i]->pos.y()<<", "<<odom_data_[i]->pos.z()<<"]");

                    // ROS_INFO_STREAM("Create odom's cost func--score:"<<odom_data_[i]->score<<" config:"<<config_.var_odom_t
                ...
                for(int j = 0; j < gps_data_.size(); ++j){
                    if(gps_data_[j]->time == odom_data_[i]->time) {
                        // ROS_INFO_STREAM("Create GPS's cost func:["<<std::fixed<<std::setprecision(6)<<i<<":"<<gps_data_[j]->pos.x()<<", "
            ...
            ROS_INFO_STREAM("ceres::summary:"<<summary.BriefReport());
            ...
            for(int i = 0; i < length; ++i){
                ...
                optimized_result->pos = Eigen::Vector3d(to_optimized_t[i][0], to_optimized_t[i][1], to_optimized_t[i][2]);

                // ROS_INFO_STREAM("Optimizing--pos:["<<to_optimized_t[i][0]<<", "<<to_optimized_t[i][1]<<", "<<to_optimized_t[i][2]<<"]");
                optimized_result->q = Eigen::Quaterniond(to_optimized_q[i][0],to_optimized_q[i][1],to_optimized_q[i][2],to_optimized_q[i][3]);
                // ROS_INFO_STREAM("optimized_data["<<i<<"]:("<<to_optimized_t[i][0]<<", "<<to_optimized_t[i][1]<<", "<<to_optimized_t[i][2]<<")");
                ...
                if(i == length - 1){
                    ...
                    ROS_INFO_STREAM("CeresOptimizer::Optimize()-W_GPS_TO_BODY:"<<W_GPS_TO_BODY(0,3)<<", "<<W_GPS_TO_BODY(1,3)<<", "<<W_GPS_TO_BODY(2,3));
                }
            }
            UpdateOptimizedPath();
            odom_mutex_.unlock();
...
void CeresOptimizer::AddPoseToNavPath(nav_msgs::Path& path, POSEPtr pose, std::string frame_id){
    ...
    // ROS_INFO_STREAM("cur pose: "<<std::fixed<<std::setprecision(6)<<
    //                 pose->pos.x()<<", "<<pose->pos.y()<<", "<<pose->pos.x());
    // ROS_INFO_STREAM("init_pos: "<<std::fixed<<std::setprecision(6)<<
    //                 init_pos_.x()<<", "<<init_pos_.y()<<", "<<init_pos_.x());
    // ROS_INFO_STREAM("path_pos: "<<std::fixed<<std::setprecision(6)<<
