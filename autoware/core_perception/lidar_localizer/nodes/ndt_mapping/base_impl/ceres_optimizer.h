...
namespace mapping{
  struct GPSCostFunction{
    ...
    bool operator()(const T* tj, T* residuals )const{
        ...
        // ROS_INFO_STREAM("gps-cost func     -tj:["<<tj[0]<<", "<<tj[1]<<", "<<tj[2]<<"]");
        // ROS_INFO_STREAM("gps-cost func-default:["<<x_<<", "<<y_<<", "<<z_<<"]");
        // ROS_INFO_STREAM("gps-cost    residuals:["<<residuals[0]<<", "<<residuals[1]<<", "<<residuals[2]<<"]");
        return true;
    }
  
    static ceres::CostFunction* create(const double x, double y, double z, double var){
  ...
  struct OdomCostFunction{
    OdomCostFunction(const double tf_x, const double tf_y, const double tf_z,
                     const double tf_qw,const double tf_qx,const double tf_qy, const double tf_qz,
                     const double var_t,const double var_q):
        tf_x_(tf_x), tf_y_(tf_y),tf_z_(tf_z),
        tf_qw_(tf_qw),tf_qx_(tf_qx),tf_qy_(tf_qy),tf_qz_(tf_qz),
        var_t_(var_t),var_q_(var_q){}
    ...
    bool operator()(const T* w_q_i, const T *ti, //t1  i  pose of gps
        ...
        t_w_ij[1] = tj[1] - ti[1];
        t_w_ij[2] = tj[2] - ti[2];
        // ROS_INFO_STREAM("odom-cost func-ti:["<<ti[0]<<", "<<ti[1]<<", "<<ti[2]<<"]");
        // ROS_INFO_STREAM("odom-cost func-tj:["<<tj[0]<<", "<<tj[1]<<", "<<tj[2]<<"]");
        // ROS_INFO_STREAM("odom-cost func-default:["<<tf_x_<<", "<<tf_y_<<", "<<tf_z_<<"]");
        T w_q_i_inverse[4];
        ...
        // ROS_INFO_STREAM("odom-cost   residuals:["<<residuals[0]<<", "<<residuals[1]<<", "<<residuals[2]<<"]");
    ...
    static ceres::CostFunction* create(const double tf_x, const double tf_y, const double tf_z,
        ...
        return (new ceres::AutoDiffCostFunction<OdomCostFunction, 6, 4, 3, 4, 3>(
            new OdomCostFunction(tf_x,tf_y,tf_z,tf_qw,tf_qx,tf_qy,tf_qz,var_t,var_q)
        ));
    }
...
class CeresOptimizer{
 public:
  ...
  void Optimize();
  bool GetOptimizedResult(POSEPtr& result);
  
  void UpdateOptimizedPath();

//   void SetInitPostion(const Eigen::Vector3d& init_pos){ 
//       init_pos_ = init_pos; 
//       ROS_INFO_STREAM("Visual-path's init pos:["<<std::fixed<<std::setprecision(6)<<init_pos_.x()<<", "<<init_pos_.y()<<", "<<init_pos_.z()<<"]");
//   }
  void AddPoseToNavPath(nav_msgs::Path& path, POSEPtr pose, std::string frame_id = "world");
